...
select s.store_id "ID магазина", count(*) "Количество покупателей", city "Город", concat(st.last_name, ' ', st.first_name) "Имя сотрудника"
from customer c
...
select title "Название фильма", rating "Рейтинг", c."name" "Жанр", f.release_year "Год выпуска", l."name" "Язык", count(r.rental_id) "Количество аренд", sum(amount) "Общая стоимость аренды"
from film f
...
group by f.film_id, c.category_id,l.language_id
having count(r.rental_id) = 0 and sum(amount) is null
order by 1
